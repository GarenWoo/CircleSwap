...
export default {
  ...
  methods: {
    ...
    // 改：将所有网络 this.MessageTransmitterAddr 移动到 setTargetChain 函数中
    // 修复部分字段赋值的末尾以逗号结束的情况，改为 ";"
    getNetworkInfo() {
      switch (this.chainId) {
            // mainnet
            case 1:
    ...
    // 改：将 this.MessageTransmitterAddr 由 getNetworkInfo() 挪到本函数中：
    // 更新了 fuji 测试链的settler合约地址，其他链地址也更新完成
    setTargetChain() {
      switch (this.TargetChainIdInHexa) {
        ...
        this.settlerAddress = '0x1e87A2D1D8E06aB74A10DE8147f31E05a7f60cFd';
        ...
        this.settlerAddress = '0x7cD5bdB0C9FfA3A5eace3F0f94883eddEa94940A';
        ...
        this.settlerAddress = '0x8a882090DbdFaAf3039dfb52825306Ff33a53fd2';
        ...
        this.settlerAddress = '0x7BdBE8630C134960D037033339A202f5e2Fb8f1E';
        ...
        this.settlerAddress = '0x83C4FafDC05193F1f99788C5e4494788DdaBB186';
    ...
    async switchToOtherNetwork() {
      ...
      try {
        ...
        if (this.switchForCrossChainSwapBool == true) {
            ...
            this.tips = `Switched to ${this.chainName} successfully. Follow Metamask and proceed.`;
            }
          }
    ...
    // 改：新增函数 getTargetChainContractABI ，用于对目标链上的合约进行初始化
    async getTargetChainContractABI(contractAddress) {
      try {
        const fetchedABI = await fetch(`${this.TargetChainScanApiURL}?module=contract&action=getabi&address=${contractAddress}&apikey=${this.TargetChainScanApiKey}`);
        const contractABI = await fetchedABI.json();
        if (contractABI.status === '1') {
          return JSON.parse(contractABI.result);
        } else {
          this.errorMessage = `Failed to get target contract ABI for address: ${contractAddress}, status: ${contractABI.status}`;
          console.error(this.errorMessage);
          return null;
        }
      } catch (error) {
    ...
    // 3. 去掉 situationNum;
    // 4. STEP 5 最开始的判断条件若不满足时，则增加返回值 dataPack，内含多个参数，随后调用 crosschainDataTransfer 将 dataPack 发送到服务器端;
    ...
    async crosschainSwap() {
      if (this.targetChainChecker() != true) {
        ...
        this.tips = "Follow Metamask's guide and complete the crosschain swap. Leaving the page is not suggested.";
        // Set all values of the variables about target chain.
        this.setTargetChain();
        
        // 改：修改代码位置，将 destinationAddressChecker 函数 和 destination 的声明前置于 gasEstimateAndPayableChecker 的执行；
        // 增加声明 recipient 作为真实的接收者（即最终接收者）与 直接接收者 destnation 作区分（destination 也可能 settler 合约，通过合约再发送给最终接收者）
        this.destinationAddressChecker();
        // Destination address on the target chain.
        let destination = this.destinationAddrCrosschain;
        let recipient;
        if (this.swapToMeBool == true) {
          recipient = this.account;
        } else if (this.swapToMeBool == false) {
          recipient = this.destinationAddrInput;
        }
        // 改：此处改为数组接收 gasEstimateAndPayableChecker 的两个变量，而非单个变量，改用 let 声明。并增加函数的输入参量 destination。
        // Estimate the total gas cost.
        let { payableBool, gasCostEstimated } = await this.gasEstimateAndPayableChecker(recipient);
        console.log(`payableBool: ${payableBool}`);
        ...
        console.log(`Signature: ${attestation}`);

        console.log(`this.swapToMeBool: ${this.swapToMeBool}; payableBool: ${payableBool}`);

        // STEP 5: If this.swapToMeBool == true && payableBool == true, go ahead to this step. if the condition is not satisfied, return dataPack to server.
        ...
        if (this.swapToMeBool == true && payableBool == true) {
        console.log('1');
        ...
        this.MessageTransmitterContract = new ethers.Contract(this.MessageTransmitterAddr, await this.getTargetChainContractABI(this.MessageTransmitterAddr), targetChainSigner);
        console.log('3');
        const receiveTxGas = await this.MessageTransmitterContract.estimateGas.receiveMessage(messageBytes, attestation);
        console.log('4');
        const receiveTx = await this.MessageTransmitterContract.receiveMessage(messageBytes, attestation, { gasLimit: receiveTxGas });
        ...
        console.log(`txURL: ${this.txURL}`);
          ...
          // 改：新声明的局部变量 targetChainIdInHexa 与 settlerAddress 分别用于接收全局变量 TargetChainIdInHexa 和 settlerAddress 的值，并被放入datapack 中
          const targetChainIdInHexa = this.TargetChainIdInHexa;
          const settlerAddress = this.settlerAddress;
          const dataPack = { recipient, gasCostEstimated, targetChainIdInHexa, settlerAddress, messageBytes, attestation };
          this.txURL = await this.crosschainDataTransfer(dataPack);
        }
      }
      this.inProcessBool = false;
      this.tips = "Crosschain swap has finished.";
    },
    ...
    destinationAddressChecker() {
      ...
      // 改：判断条件修改，先判断是否前端获取的destinationJudge为非空，若否则报错。
      if (destinationJudge != null) {
    ...
    async gasEstimateAndPayableChecker(recipient) {
      ...
      await fetch(`${this.TargetChainScanApiURL}?module=account&action=balance&address=${recipient}&tag=latest&apikey=${await this.TargetChainScanApiKey}`)
      .then(response => {
        if (response.ok) {
      ...
      let balanceOfRecipientString = balanceOfRecipient.toString();
      let balanceOfRecipientRes = ethers.BigNumber.from(balanceOfRecipientString);
      // 改：为 getGasPrice 补充前缀 this. 并将代码前置于 fetch 之后
      const totalGasPrice = await this.getGasPrice();
      console.log(`gasPrice: ${totalGasPrice}`);
      
      // 改：指定 gasAmountEstimated 的值
      gasAmountEstimated = ethers.BigNumber.from('235500');

      // 改：此处 gasCostEstimated 和 payable 改为用 let 声明, 并改用 payable 作为是否接收者可否负担起 gas 的 bool 变量;
      // 此外，增加返回值 payable 和 gasCostEstimate
      let gasCostEstimated = totalGasPrice.mul(gasAmountEstimated);
      console.log(`gasCostEstimated:${gasCostEstimated} & balanceOfRecipientRes: ${balanceOfRecipientRes}`);
      let payableBool;
      if (balanceOfRecipientRes.gt(gasCostEstimated))  {
        payableBool = true;
      } else {
        payableBool = false;
      }
      ...
      //   payableBool = false;
      // }
      return { payableBool, gasCostEstimated };
    },
    ...
    async getGasPrice() {
      ...
      try {
        ...
        TxLoop:
        while (transactionCounter < 2 && currentBlockNumber > 0) {
...
// ---------------------------------------------------- Post to Server ----------------------------------------------------
    ...
    async crosschainDataTransfer (data) {
      let counter = 0;
      while (counter < 30) {
      try {
        const response = await fetch('http://localhost:3000/crosschain-dataTransfer', {
          method: 'POST',
          headers: {
    ...
    // async gasEstimatedDataTransfer () {
    //   let counter = 0;
    //   while (counter < 30) {
    //     try {
    //       const response = await fetch('http://localhost:3000/gasEstimated-dataTransfer');
    //       if (response.ok) {
    //         const data = await response.json();
    //         const gasAmountObject = data.gasAmountEstimated;
    //         const gasAmountEstimated = ethers.BigNumber.from(gasAmountObject.hex);
...
<template>
  <div>
    <div style="display: flex; align-items: center;">
      ...
      />
      <!-- 标题 -->
      <h1 style="font-size: 36px; font-weight: bold; color: orange; margin-top: 30px;">Circle Frictionless Cross-chain Swap</h1> <span style="color: gray; font-size: 18px; margin-top: 35px; margin-left: 18px;">v0.2 Beta</span>
    </div>
  </div>
  ...
  <div >
    ...
    Wallet Address : {{  account }}
      ...
      <div >
        ...
        <!-- 改：增加一个选择结算receive货币类型的下拉选单 -->
        <!-- <label for="currencyOutType" style="color: #3344ff; font-weight: bold; font-size: 18px; margin-left: 10px;">Currency Type:</label>
        <select id="currencyOutType" v-model="currencyOutType" :disabled="unconnectedDisabled" style="margin-left: 19px;">
        <option value="USDC" type="text">USDC</option>
        <option value="Coin" type="text">{{ coinSymbol }}</option>
        </select> -->
        <br />
        <!-- 改：修正以下两行代码中 for 和 id 的值，改为"targetChainSelection" -->
        <label for="targetChainSelection" style="color: mediumslateblue; font-weight: bold;">Target Network:</label>
        <select id="targetChainSelection" v-model="TargetChainIdInHexa" @change="setTargetChain" :disabled="unconnectedDisabled" style="margin-left: 19px;">
        <option value="0x1" type="text">Ethereum Mainnet</option>
        <option value="0xa86a" type="text">Avalanche C-Chain</option>
        <option value="0xa" type="text">Optimism Mainnet</option>
        <option value="0xa4b1" type="text">Arbitrum One</option>
        ...
        <!-- 改：增加一个选择pay的货币类型的下拉选单 -->
        <!-- <label for="currencyInType" style="color: #3344ff; font-weight: bold; font-size: 18px; margin-left: 10px;">Currency Type:</label>
        <select id="currencyInType" v-model="currencyInType" :disabled="unconnectedDisabled" style="margin-left: 19px;">
        <option value="USDC" type="text">USDC</option>
        <option value="Coin" type="text">{{ coinSymbol }}</option> -->
        <!-- </select> -->
        <br />
